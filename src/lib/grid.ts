...
export default class LuckyGrid extends Lucky {
  ...
  private initWatch(): void {
    ...
    this.$watch("button", (newData: ButtonType, oldData: ButtonType) => {
      let willUpdate = [],
        btnIndex = this.cols * this.rows - 1;
      // 首次渲染时, oldData不存在
  ...
  public init(willUpdateImgs: Array<CellImgType[] | undefined>): void {
    ...
    const endCallBack = (): void => {
      ...
      if (button && config.canvasElement)
        config.canvasElement.onclick = (e) => {
   ...
   */
  private async loadAndCacheImg(prizeIndex: number, imgIndex: number, callBack: () => void) {
  ...
  protected draw(): void {
    ...
    this.cells.forEach((prize, cellIndex) => {
      ...
      const shadow: any = (isActive ? _activeStyle.shadow! : prize.shadow || _defaultStyle.shadow!)
      ...
      prize.fonts &&
        prize.fonts.forEach((font) => {
          // 字体样式
          let style =
            isActive && _activeStyle.fontStyle ? _activeStyle.fontStyle : font.fontStyle || _defaultStyle.fontStyle;
          // 字体加粗
          let fontWeight =
            isActive && _activeStyle.fontWeight ? _activeStyle.fontWeight : font.fontWeight || _defaultStyle.fontWeight;
          // 字体大小
          let size =
   ...
   * @param isActive
   */
  private handleBackground(x: number, y: number, width: number, height: number, background: string, isActive = false) {
  ...
  private run(num: number = 0): void {
    ...
    if (interval >= _defaultConfig.accelerationTime && prizeFlag !== undefined) {
      ...
      while (++i) {
        ...
        let currSpeed =
          quad.easeOut(this.FPS, this.stopIndex, endIndex, _defaultConfig.decelerationTime) - this.stopIndex;
  ...
  private getGeometricProperty([x, y, col, row]: number[]) {
    ...
    let res = [this.prizeArea!.x + (cellWidth + gutter) * x, this.prizeArea!.y + (cellHeight + gutter) * y];
   ...
   * @return 返回相对宽度
   */
  private getWidth(width: string | number | undefined, col: number = 1) {
    if (isExpectType(width, "number")) return width as number;
    if (isExpectType(width, "string"))
      return this.changeUnits(width as string, this.cellWidth * col + this._defaultConfig.gutter * (col - 1));
    return 0;
  }
   ...
   * @return 返回相对高度
   */
  private getHeight(height: string | number | undefined, row: number = 1) {
    if (isExpectType(height, "number")) return height as number;
    if (isExpectType(height, "string"))
      return this.changeUnits(height as string, this.cellHeight * row + this._defaultConfig.gutter * (row - 1));
    return 0;
  }
